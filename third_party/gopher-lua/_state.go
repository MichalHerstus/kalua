...
// SetHook installs a debug hook on the LState. event is a string of hook event
// characters: 'l' (line), 'c' (call), 'r' (return); a count > 0 installs an
// instruction count hook. Installed hooks are invoked by the interpreter at
// the corresponding boundaries. Pass nil callback with an empty event to clear
// all hooks.
func (ls *LState) SetHook(callback *LFunction, event string, count int) error {
	// Clear all existing hooks first.
	ls.lhook, ls.chook, ls.rhook, ls.cthook = nil, nil, nil, nil
	ls.prevline = 0
	if callback == nil {
		return nil
	}
