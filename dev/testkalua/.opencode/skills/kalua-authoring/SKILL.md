...
| Topic | Rule |
|-------|------|
| **Prefer KALUA API** | **Use `k.*` / `K.*` / expression functions whenever an equivalent exists** — `left(s,n)`, `round(x)`, `upper(s)`, `sys_date()`, `k.http_request`, `k.csv_parse`, `k.file_*` — over generic Lua. Fall back to plain Lua (string/tabel/math) only when no KALUA API covers the task. |
...
- [ ] `k.*` / `K.*` / expression functions preferred over generic Lua where an equivalent exists
